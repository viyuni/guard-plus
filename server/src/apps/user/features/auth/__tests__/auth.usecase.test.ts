...
it('用户注册验证 UID 后会创建用户并回放迁移积分及奖励', async () => {
  ...
  expect(replayByUserId).toHaveBeenCalledWith('user-id');
});

it('用户注册事务失败时不会消费已完成的 UID 验证', async () => {
  const migrationError = new Error('migration failed');

  const { consumeChallenge, replayByUserId, useCase } = await createUseCase(matchedChallenge(), {
    migrationError,
  });

  await expect(useCase.register(input, credential)).rejects.toBe(migrationError);
  expect(consumeChallenge).not.toHaveBeenCalled();
  expect(replayByUserId).not.toHaveBeenCalled();
});
